...
export default function DashboardPage() {
  ...
  const totalTrackedVolume = [...myPurchases, ...mySubscriptions].reduce((sum, item) => sum + Number(item.amount || 0), 0);
  const activeAutomationCount = Number(Boolean(preferences.autoBuyEnabled)) + Number(Boolean(preferences.autoListEnabled));
      ...
      <div className="dashboard-shell min-h-screen bg-main-bg">
        ...
        <main className="relative mx-auto max-w-5xl px-6 pb-16 pt-12 sm:px-8">
          <div className="glass-panel-strong rounded-[2rem] p-8 text-center sm:p-12">
            ...
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-text-secondary">Dashboard access</p>
            <h1 className="mt-3 text-3xl font-semibold tracking-tight text-text-main">Connect your wallet</h1>
        ...
        <section className="glass-panel-strong mb-6 rounded-[2rem] p-6 sm:p-8">
          <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-2xl">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-text-secondary">Agent control center</p>
              ...
              <p className="mt-4 max-w-xl text-base leading-7 text-text-secondary">
                Track marketplace flow, tune automation, and review agent activity from one dashboard with clearer structure and stronger accessibility.
              </p>
            </div>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="glass-inset rounded-[1.5rem] p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-text-secondary">Tracked volume</p>
              <p className="mt-3 text-2xl font-semibold text-text-main">${totalTrackedVolume.toFixed(2)}</p>
              <p className="mt-1 text-sm text-text-secondary">Purchases and subscriptions combined.</p>
            </div>
            <div className="glass-inset rounded-[1.5rem] p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-text-secondary">Automation rules</p>
              <p className="mt-3 text-2xl font-semibold text-text-main">{activeAutomationCount}/2 active</p>
              <p className="mt-1 text-sm text-text-secondary">Auto-buy and auto-list controls currently enabled.</p>
            </div>
            <div className="glass-inset rounded-[1.5rem] p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-text-secondary">Recurring services</p>
              <p className="mt-3 text-2xl font-semibold text-text-main">{activeSubscriptions}</p>
              <p className="mt-1 text-sm text-text-secondary">Active subscriptions awaiting the next billing cycle.</p>
            </div>
          </div>
        </section>
        ...
        <section className="glass-panel mt-8 overflow-hidden rounded-[2rem]">
          <div className="border-b border-slate-200/80 px-4 py-4 sm:px-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-lg font-semibold text-text-main">Marketplace records</h2>
                <p className="mt-1 text-sm text-text-secondary">
                  Review purchases, listings, and recurring subscriptions in one place.
                </p>
              </div>
              <div className="flex flex-wrap gap-2" role="tablist" aria-label="Dashboard records">
                {[
            ...
            {activeTab === "purchases" ? (
              myPurchases.length === 0 ? (
                <div className="py-14 text-center">
                  ...
                  <p className="mb-2 text-lg font-medium text-text-main">No purchases yet</p>
                  <p className="mb-5 text-text-secondary">You have not purchased any services yet.</p>
                  <Link
                    href="/marketplace"
                ...
                <div className="space-y-3">
                  {myPurchases.map((purchase) => {
                    ...
                    return (
                      <div key={purchase.id} className="glass-inset flex flex-col gap-4 rounded-[1.5rem] p-4 md:flex-row md:items-center md:justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-text-secondary">from {seller?.name || "Unknown"}</p>
                          </div>
                        </div>
              ...
              myListings.length === 0 ? (
                <div className="py-14 text-center">
                  ...
                  <p className="mb-2 text-lg font-medium text-text-main">No listings yet</p>
                  <p className="mb-5 text-text-secondary">Start earning by creating your first listing.</p>
                  <Link
                    href="/dashboard/listing/new"
                    className="focus-ring inline-flex items-center gap-2 rounded-full border border-brand bg-brand px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand-hover"
                  >
                ...
                <div className="space-y-3">
                  {myListings.map((listing) => (
                    <div key={listing.id} className="glass-inset flex flex-col gap-4 rounded-[1.5rem] p-4 md:flex-row md:items-center md:justify-between">
                      <div className="flex items-center gap-4">
                        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-light">
                      ...
                      <div className="flex items-center gap-3">
                        <span
                          className={`rounded-full px-3 py-1.5 text-xs font-medium ${
                            listing.active ? "bg-green-100 text-green-800" : "bg-slate-100 text-slate-700"
                          }`}
                        >
              ...
              <div className="py-14 text-center">
                ...
                <p className="mb-2 text-lg font-medium text-text-main">No subscriptions yet</p>
              ...
              <div className="space-y-3">
                {mySubscriptions.map((subscription) => {
                  ...
                  return (
                    <div key={subscription.id} className="glass-inset flex flex-col gap-4 rounded-[1.5rem] p-4 md:flex-row md:items-center md:justify-between">
                      <div className="flex items-center gap-4">
                        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-light">
                          <span className="text-sm font-bold text-brand">${subscription.amount}</span>
                        </div>
                        <div>
                          <p className="font-medium text-text-main">{listing?.title || `Subscription #${subscription.id.slice(-6)}`}</p>
                          <p className="text-sm text-text-secondary">
                            {subscription.planType === "annual" ? "Annual plan" : "Monthly plan"} · {seller?.name || "Unknown seller"}
                          </p>
                        </div>
                      </div>
                      <div className="flex flex-wrap items-center gap-3">
