...
export function ActivityLogPanel({ maxItems = 50, showFilters = true }: Props) {
  ...
  if (!isConnected) {
    return (
      <div className="rounded-xl border border-border-main bg-surface p-8 text-center shadow-sm">
  ...
  return (
    <div className="rounded-xl border border-border-main bg-surface shadow-sm">
      ...
      {showFilters && (
        <div className="flex gap-2 border-b border-border-main bg-gray-50/50 px-4 py-3 overflow-x-auto">
          {FILTER_OPTIONS.map((option) => {
            const count = option.value === 'ALL' 
              ? logs.length 
              : logs.filter(l => l.type === option.value).length;
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3 px-4 py-3">
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${iconData.bg} ${iconData.text}`}>
                      {iconData.icon}
                    </div>
                      ...
                      {isExpanded && log.details && (
                        <div className="mt-3 flex flex-wrap gap-2">
                          {Object.entries(log.details).map(([key, value]) => (
                            <span
                              key={key}
                              className="inline-flex items-center gap-1 rounded-lg bg-white px-2.5 py-1.5 text-xs font-medium text-text-secondary shadow-sm"
                            >
                    ...
                    {log.details && (
                      <div className={`shrink-0 text-text-secondary transition-transform ${
