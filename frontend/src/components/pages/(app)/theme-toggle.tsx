...
export function ThemeToggle() {
  ...
  return (
    ...
    >
      <span className={`absolute inset-0 rounded-full bg-brand-light opacity-0 blur-md transition-opacity duration-300 ${switching ? "opacity-100" : ""}`} />
      <span className="relative flex h-5 w-5 items-center justify-center">
        <svg
          className={`absolute h-5 w-5 transition-all duration-300 group-hover:scale-110 ${
